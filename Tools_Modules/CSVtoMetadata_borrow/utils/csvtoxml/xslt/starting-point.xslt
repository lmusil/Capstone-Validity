<xsl:stylesheet xmlns:xsl='http://www.w3.org/1999/XSL/Transform' version='1.0' xmlns:ns2="http://azgs.az.gov/2010/metadata/generator" xmlns:azgs1="http://azgs.az.gov/2010/metadata/template/v-1-2" xmlns:azgs2="http://azgs.az.gov/2010/metadata/source/v-1-3" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gml="http://www.opengis.net/gml" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:fn="http://www.w3.org/2005/xpath-functions">
...
<!--						<xsl:apply-templates select="//resource_lineage_statement"/>-->
						<xsl:choose>
							<xsl:when test="not(//resource_lineage_statement)">
								<gmd:lineage>
									<gmd:LI_Lineage>
							...
							<xsl:when test="string-length(//resource_lineage_statement) &lt; 1">
								<gmd:lineage>
									<gmd:LI_Lineage>
										<gmd:statement gco:nilReason="missing">
											<gco:CharacterString>missing</gco:CharacterString>
					...
					<gmd:CI_OnlineResource>
						<xsl:choose>
							<xsl:when test="contains(.,'{')">
								<gmd:linkage>
									<gmd:URL>
										<xsl:value-of select=" concat('http://',substring-before(substring-after(.,'http://'),'}'))"/>
									</gmd:URL>
								</gmd:linkage>
								<gmd:name>
									<gco:CharacterString>
										<xsl:value-of select="concat(_,substring-after(substring-before(.,':'),'{'))"/>
									</gco:CharacterString>
								</gmd:name>
							</xsl:when>
							...
							<xsl:otherwise>
								...
								<gmd:name>
									<gco:CharacterString>
										<xsl:value-of select="string('Downloadable File')"/>
									</gco:CharacterString>
								</gmd:name>
							</xsl:otherwise>
						</xsl:choose>
...
<xsl:value-of select="concat(_,substring-after(substring-before(.,':'),'{'))"/>
	...
	<xsl:template match="data_quality_flag">
		<!-- looks like this is never going to get called -->
		<gmd:dataQualityInfo>
			<gmd:DQ_DataQuality>
				<gmd:scope>
					<gmd:DQ_Scope>
